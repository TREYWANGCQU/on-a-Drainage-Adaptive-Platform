...
/**
 * 规范化数学公式模板库（与 ref/隧道防排水优化设计计算书.md 严格对齐）
 */
export const FormulaTemplates = {
  ...
  scsRunoff: (p: number, Ia: number, S: number, hs: number) =>
    `h_s = \\frac{(p - I_a)^2}{p - I_a + S} = \\frac{(${p.toFixed(1)} - ${Ia.toFixed(2)})^2}{${p.toFixed(1)} - ${Ia.toFixed(2)} + ${S.toFixed(2)}} = ${hs.toFixed(2)}\\ \\text{mm}`,

  scsEffectiveHead: (H: number, p: number, hs: number, h0: number) =>
    `h_0 = H + \\frac{p - h_s}{1000} = ${H.toFixed(2)} + \\frac{${p.toFixed(1)} - ${hs.toFixed(2)}}{1000} = ${h0.toFixed(4)}\\ \\text{m}`,

  beta1Factor: (kr: number, beta1: number) =>
    `\\beta_1 = 1.635 + 0.43\\lg k_r + 0.029(\\lg k_r)^2 = 1.635 + 0.43\\lg(${kr}) + 0.029(\\lg(${kr}))^2 = ${beta1.toFixed(4)}`,
  ...
  doubleMappingRadius: (phiDeg: number, R_inf: number, R_map: number) =>
    `R_{\\text{map}} = \\left(1 - \\frac{\\phi}{360^\\circ}\\right) R_{\\text{inf}} + \\frac{R_{\\text{inf}}}{\\pi}\\sin\\frac{\\phi}{2} = ${R_map.toFixed(2)}\\ \\text{m}`,
  ...
  unitDischargeSub: (kr: number, R: number, denomSum: number, q: number) =>
    `q = \\frac{2\\pi \\times ${kr} \\times ${R.toFixed(2)}}{${denomSum.toFixed(4)}} = ${q.toFixed(4)}\\ \\text{m}^3/(\\text{d}\\cdot\\text{m})`,
  ...
  terzaghiArchHeight: (gradeFactor: number, B: number, Hq: number) =>
    `H_q = 0.45 \\times 2^{\\frac{6 - \\text{grade}}{6}} \\cdot B = 0.45 \\times 2^{${gradeFactor.toFixed(2)}} \\times ${B.toFixed(2)} = ${Hq.toFixed(2)}\\ \\text{m}`,
  ...
  limitAxialForce: (Rw: number, b: number, xi_b: number, h0_sec: number, Nb: number) =>
    `N_b = R_w \\cdot b \\cdot \\xi_b \\cdot h_0 = ${Rw.toFixed(1)} \\times ${b} \\times ${xi_b} \\times ${h0_sec} \\times 10^{-3} = ${Nb.toFixed(1)}\\ \\text{kN}`,

  eccentricityDistance: (M: number, N: number, h: number, as_cov: number, e: number) =>
    `e = \\frac{M}{N} + \\frac{h}{2} - a_s = \\frac{${M.toFixed(1)}}{${N.toFixed(1)}} + \\frac{${h}}{2000} - \\frac{${as_cov}}{1000} = ${e.toFixed(4)}\\ \\text{m}`,

  limitResistMoment: (Rg: number, Ag: number, h0_sec: number, as_cov: number, Mu: number) =>
    `M_u = R_g \\cdot A_g \\cdot (h_0 - a_s) = ${Rg} \\times ${Ag} \\times (${h0_sec} - ${as_cov}) \\times 10^{-6} = ${Mu.toFixed(2)}\\ \\text{kN}\\cdot\\text{m}`,
  ...
  criticalLnRgSub: (term1: number, C0: number, denom: number, ln_rg: number) =>
    `\\ln r_g = \\frac{${term1.toFixed(5)} - ${C0.toFixed(5)}}{${denom.toFixed(5)}} = ${ln_rg.toFixed(4)}`,
  ...
  manningSubRing: (n: number, A: number, Rh: number, i: number, Q_cap: number) =>
    `Q_{\\text{cap}} = \\frac{86400}{${n}} \\times ${A.toFixed(5)} \\times ${Rh.toFixed(4)}^{2/3} \\times ${i.toFixed(4)}^{1/2} = ${Q_cap.toFixed(2)}\\ \\text{m}^3/\\text{d}`,
