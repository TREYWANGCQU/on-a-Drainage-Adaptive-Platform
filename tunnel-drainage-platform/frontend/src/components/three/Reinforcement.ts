...
export class ReinforcementManager {
  ...
  /**
   * @param start_chainage 分区起点里程
   * @param end_chainage 分区终点里程
   * @param delta_l_min 最小纵向间距
   * @param c_ring 单环最大布设数量
   */
  constructor(start_chainage: number, end_chainage: number, delta_l_min: number, c_ring: number) {
    // 读取起点与终点里程，解算纵深里程并实施极值预分配
    const L_max = end_chainage - start_chainage;
...
export class RockBoltGenerator {
   ...
   */
  constructor(start_chainage: number, end_chainage: number, delta_l_min: number, c_ring: number) {
    // 依据极限公式 N_max = ceil(L_max / delta_l_min) * C_ring 申请连续矩阵空间
    const L_max = end_chainage - start_chainage;
    const nMax = Math.ceil(L_max / delta_l_min) * c_ring;
    ...
    this.mesh.count = 0; // 依赖后续解算模块动态赋予 N_current
  }
