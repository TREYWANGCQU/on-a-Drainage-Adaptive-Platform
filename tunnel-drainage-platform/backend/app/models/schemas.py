...
class SingleTubeSchema(BaseModel):
    ...
    c: float = Field(..., description="隧道中心埋深 (m)")
    ...
    rebar_type: Optional[str] = Field("HRB400", description="钢筋类型（物理引擎统一HRB400）")
    Ag: float = Field(..., description="配筋面积 (mm² 或 m²)")

    P_crit: float = Field(0.0, description="临界输入外水压力 (kPa)")
...
class DoubleTubeSchema(BaseModel):
    ...
    K1: float = Field(..., description="初期支护渗透系数 (m/d)")
    K2: float = Field(..., description="二次衬砌渗透系数 (m/d)")
    ha: float = Field(0.0, description="双线低水位 / 历史别名 (m)")
