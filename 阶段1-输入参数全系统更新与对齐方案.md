...
| 类别 | 参数 Key | 标准别名 (Alias) | 中文名称 | 物理单位 | 默认值 | 作用与推导规则 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **空间分区** | `start_chainage` | - | 分区起点里程 | m | `0.0` | 计算分区长度 $L = \text{end\_chainage} - \text{start\_chainage}$ |
| | `end_chainage` | - | 分区终点里程 | m | `47.0` | 约束 $\text{end\_chainage} > \text{start\_chainage}$ |
| **拓扑工况** | `tunnel_type` | - | 隧道洞型 | - | `'single'` | `'single'` / `'double'` 枚举 |
| | `D_spacing` | - | 双洞中心间距 | m | `40.0` | 仅当 `tunnel_type == 'double'` 时参与双洞地下水流场影响计算 |
| **水力学** | `p_mm` | - | 年降雨量 | mm | `1000.0` | SCS-CN 降雨下垫面补给计算 |
| | `cn_condition` | - | 灌溉条件 | - | `'灌溉良好'` | 选取 CN 曲线值 |
| | `land_use` | - | 用地类型 | - | `'居住地'` | 选取 CN 曲线值 |
| | `H` | `h` | 初始静水位水头 | m | `120.0` | 算例水头与工况判别（$r_0 / H \ge 0.062$ 为低水位） |
| | `k_r` | `K` | 围岩渗透系数 | m/d | `0.15` | 水文地质传导参数 |
| | `k_s` | `K2` | 二衬渗透系数 | m/d | `0.000864` | 防水衬砌渗透参数 |
| | `k_p` | `K1` | 初支渗透系数 | m/d | `0.00864` | 支护透水参数 |
| | `k_g` | `Kg` | 注浆圈渗透系数 | m/d | `0.00864` | 注浆堵水圈渗透参数 |
| | `r_0` | `r` | 二衬内半径 | m | `7.95` | 衬砌内边界，参与推导 $ww, hh, tt, depth$ |
| | `r_s` | `r1` | 二衬外半径 | m | `8.35` | 衬砌外边界 |
| | `r_p` | `r2` | 初支外半径 | m | `8.57` | 初支外边界 |
| | `r_g` | `rg` | 注浆圈外半径 | m | `8.57` | 堵水圈外边界，解算临界注浆半径 $r_{g\_crit}$ |
| **结构力学** | `h_1` | `c`, `depth`, `ha` | 隧道中心埋深 | m | `130.0` | 导出力学拱顶埋深 $depth = h_1 - (r_s+r_0)/2$ |
| | `grades` | - | 围岩级别 | - | `4` | 支持数值 (1-6) 或罗马数字 ('I'-'VI') |
| | `concrete_grade`| - | 混凝土级别 | - | `35` | 支持数值 `35` 或字符串 `'C35'` |
| | `Ag` | - | 配筋面积 | mm² | `1000.0` | 引擎自动换算：若 $< 1.0$ 视作 m² 并乘 $10^6$ 转成 mm² |
| | `as_mm` | - | 保护层厚度 | mm | `50.0` | 截面受力安全系数验算参数 |
| | `tol_safety_factor`| -| 容许安全系数 | - | `2.0` | 决定是否触发临界注浆堵水反算的阈值 |
| **排水管设计**| `gamma` | - | 水重度 | kN/m³| `10.0` | 水压力换算 |
| | `S_min` | - | 环向管最小间距 | m | `3.0` | 排水管选型建议约束 |
| | `S_max` | `S_code_max`| 环向管最大间距 | m | `10.0` | 排水管选型建议约束 |
| | `n_long` / `i_long` | `I_long` | 纵向管糙率/坡降 | - | `0.012`/`0.02`| 水力排水过流能力计算 |
| | `n_ring` / `i_ring` | `I_ring` | 环向管糙率/坡降 | - | `0.012`/`0.73`| 水力排水过流能力计算 |
| | `n_lat` / `i_lat` | `I_lat` | 横向管糙率/坡降 | - | `0.012`/`0.01`| 水力排水过流能力计算 |
| | `d_long0`/`d_ring0`/`d_lat0` | `default` | 各管初始管径 | m | `0.10`/`0.05`/`0.10` | 排水选型起始迭代基线 |

#### 2. 明确弃用的计算输入参数 (Deprecated Parameters)

| 参数名称 | 原字段 Key | 废弃原因与替代方案 |
| :--- | :--- | :--- |
| **隧道高宽比 h/w** | `aspect_ratio` | **物理力学计算层彻底弃用**。`optimizedDesign.py` 强制要求高宽均由 $r_s+r_0$ 自动推导（高宽比固定为 1:1），不再修改 $hh$。 |
| **钢筋类型** | `rebar_type` | **物理力学计算层弃用**。`optimizedDesign.py` 内受力安全系数计算函数 `mc.get_safety_factor()` 统一硬编码为 `"HRB400"`。 |
| **是否双侧排水** | `double_side` | **排水计算逻辑弃用**。引擎管径与间距求解已内置标准排水模式，该开关不影响水力或力学解算结果。 |
| **设计涌水量折减系数**| `beta2`, `q_reduction` | **物理计算层彻底废弃**。算法逻辑中无任何调用点，全系统清理相关字段。 |

#### 3. 特别说明的参数 (Special Remarks)

> [!IMPORTANT]
> **隧道高宽比 `aspect_ratio` (h/w) 特别说明**:  
...
| :--- | :--- | :--- | :--- | :--- |
| **一、后端引擎对齐** | `TSK-BE-01` | `backend/app/services/drainage_engine.py` | 1. 对齐 `parConPre` 几何推导与中心埋深推导；<br>2. 移除 `aspect_ratio` 修改 `hh` 逻辑；<br>3. 钢筋类型硬编码 `"HRB400"`；<br>4. 彻底清理 `beta2` 废弃代码。 | 无 |
| | `TSK-BE-02` | `backend/app/models/schemas.py` | 清理 Pydantic Schema 中的废弃字段，同步最新字段默认值与校验范式。 | `TSK-BE-01` |
...
| **三、工具与台账交互** | `TSK-FE-04` | `frontend/src/utils/excelIO.ts` | 调整 Excel 导入导出 `fieldMapping` 字典，更新模板示例行与兼容解析。 | `TSK-FE-01` |
| | `TSK-FE-05` | `frontend/src/views/ParameterDatabase.vue` | 优化参数数据库对标抽屉与载入逻辑。 | `TSK-FE-01` |
| **四、3D 渲染与对比** | `TSK-FE-06` | `frontend/src/components/three/Viewer3D.vue` | 保持 `aspect_ratio` 仅作用于 3D 几何建模网格拉伸，不向后端力学引擎透传。 | `TSK-FE-02` |
| | `TSK-FE-07` | `frontend/src/views/Dashboard.vue` & `CompareView.vue` | 校验快照发送 payload，确保云计算请求数据 100% 契合 `drainage_engine.py`。 | `TSK-BE-01` |
