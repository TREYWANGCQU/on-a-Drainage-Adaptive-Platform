...
#### 3.2 排水管网标注双态区分 (`DrainagePipeGenerator.ts`)
在 `updateAnnotations` 中，完善文本前缀与样式逻辑：
* `viewMode === 'original'`：标注前缀 `[原始]`，颜色 `#38bdf8`。
* `viewMode === 'critical'`：
  * 若存在临界加固解：标注前缀 `[临界]`，颜色 `#ff9900`，使用 `critical_state` 的推荐管径与间距。
  * 若不存在临界加固解 (`isCriticalEmpty === true`)：生成醒目绿框标注 `[临界: 安全储备足够无需加固]`。
* `viewMode === 'all'`（主视角）：若存在 `critical_state`，可交错绘制【原始】与【临界】双气泡标注。
...
| **管网标注与探针** | 1. 【排水管网参数标注】明确区分 `[原始]` 与 `[临界]` 标签。<br>2. 【最不利探针】与受力单元看板能准确反映选中状态（原始或临界）的 $K, M, N$。 | 切换 3D 模式与探针选项，核对 3D 空间 Sprite 标注文字与悬浮看板。 |
